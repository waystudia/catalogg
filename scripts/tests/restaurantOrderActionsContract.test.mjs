...
describe('restaurant order action contract', () => {
  ...
  it('shows the restaurant cash-payment gate before allowing QR verification', async () => {
    ...
    assert.match(panel, /disabled=\{!driverAtRestaurant \|\| !driverOrderPaymentConfirmed \|\| isConfirmingCash\}/);
    ...
    assert.match(driver, /pickupBlocked = waitingForRestaurantSettlement \|\| waitingForQr/);
    assert.match(driver, /Я передал \$\{formatPrice\(settlement\.restaurantOrderAmount\)\} за заказ/);
    assert.match(driver, /Ожидайте, пока \{terms\.place\} подтвердит получение суммы заказа/);
    assert.match(driver, /После завершения расчёта появится QR/);
  });
  ...
  it('lets an authenticated restaurant irreversibly delete test or pre-activation orders', async () => {
    ...
    const preactivationRealOrders = await read('supabase/migrations/20260810071859_restaurant_preactivation_real_orders.sql');
    ...
    assert.match(panel, /canDeleteOrder &&/);
    ...
    assert.match(api, /rpc\('delete_restaurant_preactivation_order'/);
    ...
    assert.match(preactivationRealOrders, /if target_legal_status = 'active'[\s\S]*preactivation_order_deletion_not_allowed/);
    ...
    assert.match(catalogShell, /order\.isTestOrder \|\| canDeleteOrders/);
  ...
  it('shows a confirmed trash action on test or pre-activation restaurant order cards', async () => {
    const workspace = await read('src/features/restaurant-admin/RestaurantAdminWorkspace.tsx');

    assert.match(workspace, /group\.orders\.map\(\(order\) => \(/);
    assert.match(workspace, /order\.isTestOrder \|\| canDeletePreactivationOrders/);
  ...
  it('reports driver-pool publication without claiming every driver received it', async () => {
    const panel = await read('src/features/restaurant-admin/OrderDetailsPanel.tsx');

    assert.match(panel, /availableDrivers\.length > 0/);
    assert.match(panel, /Заказ опубликован для \$\{availableDrivers\.length\} доступных водителей/);
    assert.match(panel, /Сейчас нет доступных водителей/);
    assert.doesNotMatch(panel, /Заказ отправлен всем доступным водителям/);
  });
