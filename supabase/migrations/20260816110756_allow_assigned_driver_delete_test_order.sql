-- Let an assigned test driver clear a single non-combined test order while
-- preserving the existing ledger cleanup and restaurant/admin permissions.
  ...
  if not (
    ...
    or exists (
      ...
      where delivery.order_id = target_order_id
        ...
        and delivery.order_group_id is null
        and order_row.catalog_id = target_catalog_id
        and order_row.is_test_order is true
    )
  ) then
    raise exception 'Test order access is required';
  end if;
