...
begin
  ...
  select
    d.id,
    coalesce(nullif(d.name, ''), split_part(coalesce(nullif(d.email, ''), u.email), '@', 1), 'Водитель'),
  ...
  where r.catalog_id = target_catalog_id
    and rc.is_active
  order by rc.is_primary desc, rc.priority, d.name;
end;
$$;
...
create or replace function public.remove_restaurant_courier(
  target_catalog_id uuid,
  target_driver_id uuid
)
returns boolean
...
begin
  ...
  update public.restaurant_couriers rc
  set is_active = false,
      is_primary = false
  from public.restaurants r
  where rc.restaurant_id = r.id
    and r.catalog_id = target_catalog_id
...
revoke all on function public.remove_restaurant_courier(uuid, uuid) from public, anon;
...
grant execute on function public.remove_restaurant_courier(uuid, uuid) to authenticated;
