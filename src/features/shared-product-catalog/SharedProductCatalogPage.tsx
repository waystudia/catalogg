      ...
      {formOpen && (
        <form className="shared-catalog-form" onSubmit={createProduct}>
          ...
          <div className="shared-catalog-form__grid">
            <label>Штрих‑код<span><Barcode /><input required inputMode="numeric" value={draft.barcode} onChange={(event) => setDraft((current) => ({ ...current, barcode: event.target.value.slice(0, 32) }))} placeholder="4601234567890" /></span></label>
            ...
            <label>Фотография<span className="shared-catalog-file"><ImagePlus /><input type="file" accept="image/*" capture="environment" onChange={(event) => setDraft((current) => ({ ...current, imageFile: event.target.files?.[0] ?? null }))} /><small>{draft.imageFile?.name ?? 'Сфотографировать или выбрать файл'}</small></span></label>
          </div>
